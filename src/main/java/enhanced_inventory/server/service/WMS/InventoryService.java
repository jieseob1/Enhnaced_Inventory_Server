...
//  }
  ...
  //재고 이동 => 창고 내에서 재고 이동 또는 창고 간 이동을 관리하는 기능
  @Transactional
  public Inventory moveInventory(Long inventoryId, Long newLocationId) {
    Inventory inventory = inventoryRepository.findById(inventoryId)
        .orElseThrow(
            () -> new ResourceNotFoundException("Inventory not found with id " + inventoryId));
    Location newLocation = locationRepository.findById(newLocationId)
        .orElseThrow(
            () -> new ResourceNotFoundException("Location not found with id " + newLocationId));
    return inventoryRepository.save(inventory);
  }

  //재고 실사 => 실제 재고 점검 & 시스템 상의 재고와 일치하는지 확인하는 기능 => 어떤 기능인지 추가 확인
//  @Transactional
//  public Inventory checkInventory(Long inventoryId, int newQuantity) {
//    Inventory inventory = inventoryRepository.findById(inventoryId)
//        .orElseThrow(
//            () -> new ResourceNotFoundException("Inventory not found with id " + inventoryId));
//`
...
//  }
  ...
  //재고 할당 및 예약: 출고 오더에 맞는 재고를 사전에 할당하거나 예약하는 기능
//    @Transactional
//    public Inventory allocateInventory(Long inventoryId, int allocatedQuantity) {
//        Inventory inventory = inventoryRepository.findById(inventoryId)
//                .orElseThrow(())
}
